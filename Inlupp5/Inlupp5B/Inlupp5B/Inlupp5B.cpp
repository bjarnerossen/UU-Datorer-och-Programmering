...
int main()
{
    ...
    {
        // Anropa funktionen namn_pa_fil för att bestämma vilket filnamn som ska behandlas
        filnamn = namn_pa_fil();
        cout << endl;
...
string Text::tolka()
{
    ...
    for(int i = 0; i < ANTAL_SPRAK; i++)
    {
        ...
        // Om kvadratsumman är mindre än föregående, lagra den som kvadratsumma och
        // jämför med den nya kvadratsumman i nästa varv i radloopen. Sätt sannolikt
        // språk till hittills lägsta kvadratsumma.
        if(sum[i] < kvadratsumma)
        {
            kvadratsumma = sum[i];
            sannolikt_sprak = i;
        }
...
-------------------------------------------------------
 ...
 Deluppgiften 5B gick mycket smidigare än tänkt att lösa, efter lite övning i Inlupp4B och då
 jag nu förtstog problemställningen mycket mera djupgående. Liksom i uppgift4B valde jag
 att inte avlsuta programmet efter användaren mata in ett felaktig filnamn, utan att förtsätta
 fråga efter en fil.
...
(1) Finns det någon anledning varför vi beräknar både det absoluta och det relativa histogrammet i samma funktionsanrop?
 ...
 Jag kan tänka mig att det har att göra med att hålla main() så liten som möjligt, då
 villkoren för exekveringen hamnar i anropsfunktionen istället. Detta gör att koden bli
 mer återanvändningsbar.
...
Man kunde skapa ett nytt objekt, som till exempel Text minAndraText och anropa funktionerna på det
objektet istället.
...
Det finns nog ingen anledning till varför det inte skulle gå. Det skulle göra det möjligt att
arbeta på variablerna direkt istället för att skicka parametrarna till externa funktioner.
Fördelen med att ateranvända samma funktioner som tidigare är att det gör koden inte lika
redundant, och man kan dessutom använda sig av kod som är redan fungerande och testad.
